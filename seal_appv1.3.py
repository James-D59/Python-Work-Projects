...
# === Spreadsheet Setup ===
xlsx_path = r"C:\Users\James\Desktop\work projects\Sealing App\seal_pull.xlsx"

tables_df = pd.read_excel(xlsx_path, sheet_name="Tables")
...
tk.Label(root, text="STACKS").grid(row=2, column=3)
tk.Label(root, text="BOOKS").grid(row=2, column=4)
tk.Label(root, text="LOG OUT").grid(row=2, column=5)
...
for i in range(5):
    ...
    stacks_entry = tk.Entry(root)
    stacks_entry.grid(row=row_idx, column=3, padx=5, pady=5)

    books_entry = tk.Entry(root)
    books_entry.grid(row=row_idx, column=4, padx=5, pady=5)

    logout_btn = tk.Button(root, text="LOG OUT", command=lambda i=i: log_out_action(i))
    logout_btn.grid(row=row_idx, column=5, padx=5, pady=5)
